<!--#####REQUIRES chart.js, vue-randomcolor#####-->

<template>
        <d-card class="card-small h-100 card1">
...
export default {
  ...
  mounted() {
    ...
    const colors = this.$randomColor({
      count: this.chartData.datasets.length,
      format: 'rgba',
      alpha: '1',
      luminosity: 'dark',
    });
    this.chartData.datasets.forEach((dataset, i) => {
      ...
      const bColor = `${color.slice(0, -2)}0.${color.slice(-2)}`;
      dataset.backgroundColor = bColor;// 'rgba(0,123,255,0.1)'
      dataset.borderColor = color;// 'rgba(0,123,255,1)'
      dataset.pointBackgroundColor = '#ffffff';
      dataset.pointHoverBackgroundColor = color;// 'rgb(0,123,255)'
      dataset.borderWidth = 1.5;
      if (i == 0) { dataset.borderDash = [0, 0]; } else { dataset.borderDash = [i * 3, i * 3]; }
